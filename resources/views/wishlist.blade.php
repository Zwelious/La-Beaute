                        ...
                        <div class="card border shadow-0">
                            <div class="m-4">
                                ...
                                @if ($wishlistProducts->isEmpty())
                                    <p>Your wishlist is empty. To add products to your wishlist, click on the heart icon on
                                        a product!</p>
                                @else
                                    @foreach ($wishlistProducts as $wishlist)
                                        @php
                                            $newPrice =
                                                $wishlist->DISKON > 0
                                                    ? $wishlist->HARGA - $wishlist->HARGA * ($wishlist->DISKON / 100)
                                                    : $wishlist->HARGA;
                                        @endphp

                                        <div class="row gy-3 mb-4" data-id="{{ $wishlist->ID_PROD }}"
                                            data-price="{{ $newPrice }}">
                                            <div class="col-lg-6">
                                                <div class="me-lg-5">
                                                    <div class="d-flex">
                                                        <img src="{{ $wishlist->FOTO_PROD }}" class="border rounded me-3"
                                                            style="width: 96px; height: 96px;" />
                                                        <div class="">
                                                            <a href="{{ route('shop-details', ['id_prod' => $wishlist->ID_PROD]) }}"
                                                                class="nav-link">{{ $wishlist->NAMA_PROD }}</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="">
                                                    ...
                                                    @if ($wishlist->DISKON > 0)
                                                        <del class="text-success">Rp
                                                            {{ number_format($wishlist->HARGA, 0, ',', '.') }}</del>
                                                    @endif
                                            ...
                                            <div
                                                ...
                                                <div class="float-md-end">
                                                    <a href="{{ route('shop-details', ['id_prod' => $wishlist->ID_PROD]) }}"
                                                        class="btn btn-light border px-2 icon-hover-primary add-to-cart"
                                                        data-id="{{ $wishlist->ID_PROD }}"><i
                                                            class="fas fa-shopping-cart fa-lg px-1 text-wishlist"></i></a>
                                                    <form action="{{ route('wishlist.remove') }}" method="POST"
                                                        style="display: inline;">
                                                        @csrf
                                                        <input type="hidden" name="product_id"
                                                            value="{{ $wishlist->ID_PROD }}">
                                                        <button type="submit"
